...
with tab1:
    ...
    if not games:
        ...
        for g in games:
            ...
            # status
            if state=="in":   st_html=f'<div class="game-status-live">&#9679; {g["status"]}</div>'
            elif state=="post": st_html='<div class="game-status-final">FINAL</div>'
            else:
                t=g["status"]
                if g["dt"]:
                    try: t=g["dt"].astimezone().strftime("%-m/%-d")
                    except: pass
                st_html=f'<div class="game-status-pre">{t}</div>'

            td=g["status"]
            if state=="pre" and g["dt"]:
                try: td=g["dt"].astimezone().strftime("%-I:%M %p %Z")
                except: pass
            net_html=f'<div class="game-network">{g["net"]}</div>' if g["net"] else ""
            venue_short=g["venue"].replace(" Stadium","").replace(" Field","")
...
<div class="gcard-inner" style="display:flex;align-items:center;gap:0">
  <div class="team-section">
    <div class="team-row">
      <img src="{logo(g["aa"])}" class="team-logo" onerror="this.style.opacity=.2">
      <div class="team-info">
        <div class="team-name">{g["aname"]}</div>
        <div class="team-record">{g["ar"]}</div>
      </div>
      {a_sc}
    </div>
    <div class="team-row">
      <img src="{logo(g["ha"])}" class="team-logo" onerror="this.style.opacity=.2">
      <div class="team-info">
        <div class="team-name">{g["hname"]}</div>
        <div class="team-record">{g["hr"]}</div>
      </div>
      {h_sc}
    </div>
  </div>
  <div class="game-center">
    {st_html}
    <div class="game-time">{td}</div>
    {net_html}
    <div class="game-venue">&#127967; {venue_short}</div>
  </div>
  <div class="wp-section">
    <div class="wp-label">WIN PROB</div>
    <div class="wp-bar-wrap">
      <div style="position:relative;height:100%">{a_bar}{h_bar}</div>
    </div>
    <div class="wp-teams">
      <span class="wp-away">{g["aa"]} {awp}%</span>
      <span class="wp-home">{hwp}% {g["ha"]}</span>
    </div>
  </div>
</div>
<div class="odds-strip">
  <div class="odds-cell">
    <div class="odds-type">MONEYLINE <span class="odds-grade {gc(p["mg"])}">{p["mg"]}</span></div>
    <div class="odds-main">{p["pick"]}</div>
    <div class="odds-sub">{g["aa"]} {ao} &nbsp;/&nbsp; {g["ha"]} {ho_str}</div>
  </div>
  <div class="odds-cell">
    <div class="odds-type">SPREAD <span class="odds-grade {gc(p["rg"])}">{p["rg"]}</span></div>
    <div class="odds-main">{sl}</div>
    <div class="odds-sub">Proj {p["ap"]} &#8211; {p["hp"]} &nbsp;&#183;&nbsp; {abs(p["hp"]-p["ap"]):.1f} pt margin</div>
  </div>
  <div class="odds-cell">
    <div class="odds-type">TOTAL O/U <span class="odds-grade {gc(p["ug"])}">{p["ug"]}</span></div>
    <div class="odds-main {ou_cls}">{p["ou"]} {p["tl"]}</div>
    <div class="odds-sub">Proj {p["tot"]} pts &nbsp;&#183;&nbsp; Line {p["tl"]}</div>
  </div>
</div>
</div>''')
